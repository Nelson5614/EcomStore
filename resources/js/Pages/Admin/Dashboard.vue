    ...
    <div class="p-6">
        ...
        <div class="bg-white rounded-lg shadow-md p-6 border-l-4 border-blue-500">
          <div class="flex items-center">
            ...
            <div>
              ...
              <p class="text-2xl font-bold text-gray-900">{{ statistics.totalProducts }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6 border-l-4 border-yellow-500">
          <div class="flex items-center">
            ...
            <div>
              ...
              <p class="text-2xl font-bold text-gray-900">${{ formatCurrency(statistics.revenueThisMonth) }}</p>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div class="lg:col-span-2 bg-white rounded-lg shadow-md p-6">
          <h2 class="text-lg font-semibold text-gray-900 mb-4">Monthly Revenue ({{ new Date().getFullYear() }})</h2>
          <div class="h-64 flex items-end justify-between space-x-2">
            <div v-for="(revenue, index) in monthlyRevenue" :key="index" class="flex-1 flex flex-col items-center">
              <div 
                ...
                :style="{ height: Math.max((revenue / Math.max(...monthlyRevenue, 1)) * 200, 5) + 'px' }"
                :title="`$${formatCurrency(revenue)}`"
              ></div>
              <span class="text-xs text-gray-600 mt-2">{{ getMonthName(index) }}</span>
            </div>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
          ...
          <div v-if="lowStockProducts.length > 0" class="space-y-3">
            <div v-for="product in lowStockProducts" :key="product.id" class="flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-50">
              <div>
      ...
      <div class="mt-6 bg-white rounded-lg shadow-md p-6">
        ...
        <div v-if="topSellingProducts.length > 0" class="grid grid-cols-1 md:grid-cols-5 gap-4">
          <div v-for="item in topSellingProducts" :key="item.product_id" class="text-center p-4 border border-gray-200 rounded-lg hover:shadow-md transition-shadow duration-200">
            <div class="w-16 h-16 mx-auto mb-3 bg-gray-200 rounded-lg flex items-center justify-center">
              <svg class="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path>
              </svg>
            </div>
            <p class="text-sm font-medium text-gray-900 truncate">{{ item.product?.title || 'Unknown' }}</p>
