...
class APIRequest {
	...
	function execute() {
		...
		// Add these lines to your cURL setup
	...
	function __get($ch) {
		...
		// START --- Added cURL error checking
		if ($raw_response === false) {
			$curl_error = curl_error($ch);
			$curl_errno = curl_errno($ch);
			curl_close($ch);
			error_log("Mpesa SDK cURL Error in GET ($curl_errno): $curl_error");
			return new APIResponse(0, "cURL Error: $curl_error", ""); 
		}
		// END --- Added cURL error checking
		
		$status_code = curl_getinfo($ch, CURLINFO_HTTP_CODE);
		$header_size = curl_getinfo($ch, CURLINFO_HEADER_SIZE);
		$headers = is_string($raw_response) ? substr($raw_response, 0, $header_size) : '';
		$body = is_string($raw_response) ? substr($raw_response, $header_size) : '';
		curl_close($ch);
	...
	function __post($ch) {
		...
		if ($raw_response === false) {
			...
			$curl_errno = curl_errno($ch);
			curl_close($ch);
			error_log("Mpesa SDK cURL Error in POST ($curl_errno): $curl_error");
			return new APIResponse(0, "cURL Error: $curl_error", ""); 
		}
		// END --- Added cURL error checking
	...
	function __put($ch) {
		...
		if ($raw_response === false) {
			...
			$curl_errno = curl_errno($ch);
			curl_close($ch);
			error_log("Mpesa SDK cURL Error in PUT ($curl_errno): $curl_error");
			return new APIResponse(0, "cURL Error: $curl_error", ""); 
		}
		// END --- Added cURL error checking
